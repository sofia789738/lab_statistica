...
#given an array calculate the standard deviation of the mean of its elements
def std_dev_means (my_array) :
    n = np.size(my_array)
    tot = 0
    for i in range(0,n) :
        tot = tot + my_array[i]
    ...
    variance= var/(n-1)
    std_dev = math.sqrt(variance)
    std_dev_m = std_dev / np.sqrt(n)
    return std_dev_m


#given a set of measurements, as a monodimensional array, calculate its skewness
def skewness (my_array) :
    return skew(my_array)

#given a set of measurements, as a monodimensional array, calculate its kurtosis
